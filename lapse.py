...
#######
def start_timelapse(fps, video_time, event_duration, cam_idx, height, width, output_dir, quality, remove_images):
	...
	# this is to put the video in images dir
	# os.system('ffmpeg -framerate %d -i %s -c:v libx264 -r 20 -pix_fmt yuv420p %s/%d.mp4' % (fps,image_name_format,output_dir,int(time.time())))
	
	# this is not
	os.system('ffmpeg -framerate %d -i %s -c:v libx264 -r 20 -pix_fmt yuv420p %d.mp4' % (fps, image_name_format, int(time.time())))


	if(remove_images):
		files = glob.glob('/home/omarcartera/Desktop/lapse/images/*')
...
def main():
	...
	parser.add_option("-r", "--remove-images",
					  default=True,
					  help="remove the images created during the lapse")
	...
	remove_images = int(options.remove_images)
	...
	if bool(options.preview):
		...
		start_timelapse(fps, video_duration, event_duration, cam_idx, height, width, output_dir, quality, remove_images)
